...
const Contact = () => {
  ...
  const serviceAreas = [
    ...
    { name: 'Ottawa Valley', description: 'Ottawa, Gatineau, Kanata, Orleans, Nepean, Barrhaven, Stittsville' },
    { name: 'Prince Edward County', description: 'Picton, Wellington, Bloomfield, Consecon, Carrying Place, Cherry Valley' },
    { name: 'Durham Region', description: 'Oshawa, Whitby, Ajax, Pickering, Clarington, Bowmanville, Newcastle' },
    ...
    { name: 'Kawartha Lakes', description: 'Peterborough, Lindsay, Fenelon Falls, Bobcaygeon, Omemee, Haliburton' },
    { name: 'Muskoka & Simcoe', description: 'Barrie, Orillia, Huntsville, Bracebridge, Gravenhurst, Collingwood' }
  ];
  ...
  // Initialize Maps function - separated for better error handling
  const initializeMap = () => {
    try {
  ...
  useEffect(() => {
    ...
    window.initMap = () => {
      try {
        ...
        setMapError('Google Maps API key configuration issue. Please contact support.');
        setIsMapLoading(false);
      }
    };
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="relative px-4 py-12 max-w-7xl mx-auto">
            ...
            <motion.section 
              ...
              className="mb-12"
            >
              <h2 className="font-anton text-3xl md:text-4xl text-white mb-8 text-center">Serving All of Ontario</h2>
                {isMapLoading && (
                  <div className="absolute inset-0 bg-white/10 backdrop-blur-sm flex items-center justify-center">
                    <div className="text-center">
                      <div className="w-8 h-8 border-2 border-white/30 border-t-[#ED8071] rounded-full animate-spin mx-auto mb-4" />
                      <p className="font-roboto-flex text-white/80">Loading interactive map...</p>
                    </div>
                  </div>
                )}
                ...
                {mapError && (
                  <div className="absolute inset-0 bg-white/10 backdrop-blur-sm flex items-center justify-center">
                    <div className="text-center p-8">
                      <MapPin className="w-12 h-12 text-[#ED8071] mx-auto mb-4" />
                      <h3 className="font-anton text-2xl text-white mb-2">Service Areas</h3>
                      <p className="font-roboto-flex text-white/80 mb-4">Interactive map temporarily unavailable</p>
                      <div className="space-y-2">
                        {serviceAreas.map((area, index) => (
                          <p key={index} className="font-roboto-flex text-white/70 text-sm">
                ...
                <div id="map" className="w-full h-full" style={{ opacity: mapError ? 0 : 1 }}></div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12 max-w-5xl mx-auto">
                {serviceAreas.map((area, index) => (
                  <div key={index} className="bg-transparent border-none p-2">
                    <div className="relative w-full h-48 rounded-lg mb-4 border border-white/20 overflow-hidden">
                      {mapError ? (
                        <div className="absolute inset-0 bg-white/10 backdrop-blur-sm flex items-center justify-center">
                          <div className="text-center p-4">
                            <MapPin className="w-8 h-8 text-[#ED8071] mx-auto mb-2" />
                            <p className="font-roboto-flex text-white/70 text-sm">{area.name}</p>
                          </div>
                        </div>
                      ) : (
                        <div id={`mini-map-${index}`} className="w-full h-full"></div>
                      )}
                    </div>
