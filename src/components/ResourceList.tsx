...
const ResourceGroup = ({ title, resources }: ResourceGroupProps) => (
  <div className="mb-8">
    {title && <h3 className="text-xl font-roboto-flex font-bold mb-4 text-hunter-green font-medium">{title}</h3>}
      ...
      {resources.map((resource, index) => (
        <div key={index} className="p-4 border border-hunter-green/10 rounded-lg bg-pure-white bg-opacity-70 hover:bg-opacity-90 transition-all">
          ...
          >
            ...
            <p className="text-hunter-green/80 text-sm font-roboto-flex">{resource.description}</p>
          </a>
        </div>
      ))}
    </div>
